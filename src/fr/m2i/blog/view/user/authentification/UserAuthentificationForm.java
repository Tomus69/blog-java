...
import fr.m2i.blog.view.user.UserArticleView;

public class UserAuthentificationForm extends JPanel {
	...
	private UserAuthentificationView view;
	...
	public UserAuthentificationForm(UserAuthentificationView view) {
		this.view = view;
	...
	private void authentification() throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT PASSWORD FROM USER WHERE NAME ='" + nom + "'");
		...
		if (rs.next()) {
			...
			if (motDePasse.equals(mdp)) {
				...
				UserDto userCourant = UserService.findByName(this.name.getText());
				new UserArticleView(userCourant);
				close();
			} else {
